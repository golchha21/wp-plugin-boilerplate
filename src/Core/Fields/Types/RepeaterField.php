...
class RepeaterField extends AbstractField
{
	public function render(?string $optionKey): void
	{
		...
		// Open wrapper
		echo '<div class="wppb-repeater-wrapper is-collapsed"
    data-min="' . esc_attr($this->meta['min'] ?? 0) . '"
    data-max="' . esc_attr($this->meta['max'] ?? '') . '">';

		/* HEADER */
		echo '<div class="wppb-repeater-main-header">';
    ...
    class="wppb-repeater-main-toggle"
    aria-expanded="false"
    ...
    <span class="dashicons dashicons-plus"></span>
		...
		/* BODY */
		echo '<div class="wppb-repeater-main-body">';
		...
		echo '</div>'; // .wppb-repeater-main-body
		echo '</div>'; // .wppb-repeater-wrapper
	...
	protected function renderRow(?string $optionKey, string|int $index, array $row): void
	{
		...
		foreach ($this->meta['fields'] as $subKey => $subSchema) {
			...
			echo '<label class="wppb-field-label width-2">';
			echo esc_html($definition->label);
			echo '</label>';
